...
phe.out <- "fgwas.test.snpmat.phe"  
snp.out <- "fgwas.test.snpmat.snp"
...
fgwas.simulate( phe.out, snp.out, simu_grp=1, simu_n= 800, simu_p=4000, simu_snp_rho=0.4, simu_rho=0.1, simu_sigma2= 4, 
...
ret1 <- fgwas.snpmat(tb.phe, tb.snp, Y.prefix="Y", Z.prefix="Z", covar.names=c("X_1","X_2"), fgwas.filter = T , options=list(nParallel.cpu=7));	

save(ret1, file="fgwas-test-snpmat.rdata");
summary(ret1);
plot(ret1, fig.prefix="fgwas-test-snpmat-ret1");

ret2 <- gls.snpmat(tb.phe, tb.snp, Y.prefix="Y", Z.prefix="Z", covar.names=c("X_1","X_2"), fgwas.filter = F );	

save(ret1, ret2, file="fgwas-test-snpmat.rdata");
summary(ret2);
plot(ret2, fig.prefix="fgwas-test-snpmat-ret2");
