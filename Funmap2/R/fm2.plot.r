...
#--------------------------------------------------------------
fpt.plot_qtl_pos<-function( index, dat, res, cutoff.05=NULL, cutoff.01=NULL, qtl_ps=NULL )
{
	...
	fpt.internal_plot_qtl_pos( chr_logs, marker_list, cutoff.05=cutoff.05, cutoff.01=cutoff.01, qtl_ps=qtl_ps );
}
...
#      res:  LR table.
#--------------------------------------------------------------
fpt.plot_qtl_map<-function( dat, res, cutoff.05=NULL, cutoff.01=NULL )
{
...
fpt.plot_tiled_curves<-function( dat, rows=NA, cols=NA, max_curves = NULL, selected=NULL )
{
	...
	minv <- min(as.matrix(dat$phenos_table[selected,]), na.rm=TRUE)*0.9;
	maxv <- max(as.matrix(dat$phenos_table[selected,]), na.rm=TRUE)*1.1;

	px<-as.numeric( colnames( dat$phenos_table ) );
	xmax <- max(px, na.rm=TRUE);
	xmin <- min(px, na.rm=TRUE);

	py <- length ( dat$phenos_table[1, ] );
	rx <- ceiling( py/8 );
	...
	for (i in 0:(rows-1) )
	{
		for (j in 0:(cols-1) )
		{
			...
			if ( (i*cols+j+1) <= length(selected) )
			{
				idx <- i*cols+j+1;
				px<-as.numeric( colnames( dat$phenos_table ) );
				py<-as.numeric( dat$phenos_table[selected[idx], ] );
				...
				if (FM2.get_value("curve_smooth", def=0) == 1)
...
fpt.plot_overlapping_curves<-function( dat, options=list() )
{
	...
	xmax <- max(px, na.rm=TRUE);
	xmin <- min(px, na.rm=TRUE);
	minv <- floor( min(as.matrix(dat$phenos_table), na.rm=TRUE) )*0.9;
	maxv <- ceiling( max(as.matrix(dat$phenos_table), na.rm=TRUE) )*1.1;
	...
	for (i in 1:length(dat$phenos_table[,1]) )
	{
		py<-as.numeric( dat$phenos_table[i, ] );
		px<-as.numeric( colnames(dat$phenos_table) );
		if (length(which(is.na(py)))>0)
		{
			px<-px[-(which(is.na(py)))];
	...
	if (substr( dat$name, 1, 2)=="CM" )
	{
		...
		points(xmin:xmax, colMeans(dat$phenos_table), type="o", pch=19, cex=0.5, col="red" );
	}

	if (!is.null(dat$QQ_par))
	{
		x10 <- seq(xmin,xmax,0.1);	
		y10  <- CM.get_model_mu(x10, dat$QQ_par, tmin=1, tmax=xmax  );
		lines(x10, y10, lwd=1, col="red" , lty="solid");	
	}
	...
	if (!is.null(dat$Qq_par))
	{
		x10 <- seq(xmin,xmax,0.1);	
		y10  <- CM.get_model_mu(x10, dat$Qq_par, tmin=1, tmax=xmax );
		lines(x10, y10, lwd=1, col="green" , lty="solid");	
	}
	...
	if (!is.null(dat$qq_par))
	{
		x10 <- seq(xmin,xmax,0.1);	
		y10  <- CM.get_model_mu(x10, dat$qq_par, tmin=1, tmax=xmax );
		lines(x10, y10, lwd=1, col="blue" , lty="solid");	
	}
...
fpt.plot_com_curve<-function( nMesa, nLong, f_curve_mu, dat = NULL, QQ_par=NULL, Qq_par=NULL, qq_par=NULL, simu_QQ=NULL, simu_Qq=NULL, simu_qq=NULL, xlab="Time", ylab="Model" )
{
	if ( nMesa > nLong)
	 	nLong <- nMesa + 4; 

	limit1 <- 10;	
	if (!is.null(dat)) limit1 <- max( dat$phenos_table, na.rm=TRUE );
	...
	if (!is.null(dat))
	{
		tLen <- length( dat$phenos_table[1,] );
		for (i in 1:length(dat$phenos_table[,1]) )
		{
			#lines(c(1:tLen), dat$phenos_table[i,], col=rgb(0.75,0.75,0.75), lwd=1);
