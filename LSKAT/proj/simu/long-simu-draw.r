...
bar.subplot<-function(p.dat, ylim=c(0,1),  main.title, test.no=0, legend=F)
{
	...
	{
		bar.w <- 0.8/dim(p.dat)[1];
		for(j in 1:dim(p.dat)[1])
	...
	if(legend)
	{
		...
		text( 0.7-0.02, 0.98, "SKAT(Baseline)", cex=0.6, adj=c(0, 0.5));

		rect( 0.5, 0.93, 0.62, 0.87, col=cols[2], border="black")
		text( 0.7-0.02, 0.90, "Burden(Baseline)", cex=0.6, adj=c(0, 0.5));

		rect( 0.5, 0.85, 0.62, 0.79, col=cols[3], border="black")
		text( 0.7-0.02, 0.82, "SKAT(Mean)", cex=0.6, adj=c(0, 0.5));

		rect( 0.5, 0.77, 0.62, 0.71, col=cols[4], border="black")
		text( 0.7-0.02, 0.74, "Burden(Mean)", cex=0.6, adj=c(0, 0.5));

		rect( 0.5, 0.69, 0.62, 0.63, col=cols[5], border="black")
		text( 0.7-0.02, 0.66, "LSKAT", cex=0.6, adj=c(0, 0.5));

		rect( 0.5, 0.61, 0.62, 0.55, col=cols[6], border="black")
		text( 0.7-0.02, 0.58, "Burden-LSKAT", cex=0.6, adj=c(0, 0.5));
	}
}
