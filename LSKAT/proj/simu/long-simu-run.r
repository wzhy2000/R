...
{
	snp.mat  <- simu.snp.mat(g.snp.hap1, g.snp.pos2, par$snprange, n.sample, n.rep, par$rare.cutoff, ncores)
	...
	rs <- mclapply(1:n.loop, function(i)
	{
		...
		obj.h0 <- try( longskat_est_model( phe$y, phe$cov, y.cov.time=par$y.cov.time, g.maxiter=3, intercept=par$intercept ));
...
{
	snp.mat <- simu.snp.mat(g.snp.hap1, g.snp.pos2, par$snprange, n.sample, n.loop, par$rare.cutoff,ncores)
