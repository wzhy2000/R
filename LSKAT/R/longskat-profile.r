			...
			gene.names, options=list() ) 
{
	...
	for(k in 1:length(gene.names) )
	{
		...
		ls <- longskat_gene_run(r.model, gen.mat$snp, 
							    weights.common= options$weights.common, weights.rare  = options$weights.rare, run.cpp=options$run.cpp, debug = options$debug );
