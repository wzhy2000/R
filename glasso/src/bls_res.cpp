...
BLS_res::~BLS_res()
{
    ...
    if(m_pVarsel_Coefs) delete m_pVarsel_Coefs;
    if(m_pVarsel_Ra)   delete m_pVarsel_Ra;
    if(m_pVarsel_Rd)   delete m_pVarsel_Rd;
	if(m_pVarsel_Rh2)   delete m_pVarsel_Rh2;
	if(m_pVarsel_QBest) delete m_pVarsel_QBest;
    ...
    if(m_pRefit_Coefs) delete m_pRefit_Coefs;
    if(m_pRefit_Ra) delete m_pRefit_Ra;
    if(m_pRefit_Rd) delete m_pRefit_Rd;
    if(m_pRefit_Rh2) delete m_pRefit_Rh2;
	if(m_pRefit_QBest) delete m_pRefit_QBest;
    ...
    if(m_pCovNames) delete m_pCovNames;
...
int BLS_res::InitVarsel(int mRound, int nSnpP, int SubjN, int mcmcSect, int mcmcSnps, int mcmcIter, CFmVectorStr* pCovNames)
{
	...
	m_pCovNames  = new CFmVectorStr(pCovNames);
						...
						CFmVectorStr* pVctSnp, CFmVector* pVctChr, CFmVector* pVctPos, CFmFileMatrix* pFileMat )
{
    ...
    if (bRefit)
    {
        m_pRefit_SnpName = new CFmVectorStr(pVctSnp);
        m_pRefit_SnpChr = new CFmVector(pVctChr);
        m_pRefit_SnpPos = new CFmVector(pVctPos);
        m_pRefit_Coefs = new CFmMatrix(0, 0);
        m_pRefit_Ra = new CFmMatrix(0, 0);
        m_pRefit_Rd = new CFmMatrix(0, 0);
        m_pRefit_Rh2 = new CFmVector(0, 0.0);

 		SortoutResult( pFileMat, gen_pA, y_var, nSnp, nCov, m_pRefit_Coefs, m_pRefit_Ra, m_pRefit_Rd, m_pRefit_Rh2 );

 		m_pRefit_QBest = new CFmMatrix(nSnp, 8);
    ...
    {
        m_pVarsel_SnpName = new CFmVectorStr(pVctSnp);
        m_pVarsel_SnpChr = new CFmVector(pVctChr);
        m_pVarsel_SnpPos = new CFmVector(pVctPos);
        m_pVarsel_Coefs = new CFmMatrix(0, 0);
        m_pVarsel_Ra = new CFmMatrix(0, 0);
        m_pVarsel_Rd = new CFmMatrix(0, 0);
        m_pVarsel_Rh2 = new CFmVector(0, 0.0);

		SortoutResult( pFileMat, gen_pA, y_var, nSnp, nCov, m_pVarsel_Coefs, m_pVarsel_Ra, m_pVarsel_Rd, m_pVarsel_Rh2 );

 		m_pVarsel_QBest = new CFmMatrix(nSnp, 8);
